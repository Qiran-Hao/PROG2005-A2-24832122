 ...
 */
function autoCalculateStockStatus(quantity) {
    if (quantity <= 0)
        return 'Out of Stock';
    if (quantity < 5)
        return 'Low Stock';
 ...
 */
export function updateItemByName(itemName, updatedData) {
    ...
    inventory[itemIndex] = Object.assign(Object.assign({}, inventory[itemIndex]), updatedData);
 ...
 */
export function searchItemByName(keyword) {
...
export function handleUpdateItem() {
    ...
    const newQuantity = Number(document.getElementById('update-quantity').value);
    const newPrice = Number(document.getElementById('update-price').value);
    const newIsPopular = document.getElementById('update-is-popular').checked;
